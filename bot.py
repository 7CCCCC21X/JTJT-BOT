...
async def _finalize_pending(context: ContextTypes.DEFAULT_TYPE, chat_id: int):
    ...
    for address, inline_label in entries:
        ...
        for c in chains:
            k = kind
            # 识别到代币合约:自动改用代币监控(监控该代币的所有转账),
            # 地址监控盯代币合约几乎必然不是用户本意
            if kind == "address" and det.get(c, {}).get("type") == "token":
                k = "token"
                symbol = det[c].get("symbol")
                auto_switched.append(
                    f"{CHAINS[c]['name']}" + (f" ({symbol})" if symbol else ""))
            parts.append(await create_watch(chat_id, address, c, k, label))
    if auto_switched:
        parts.append(
            "ℹ️ 检测到代币合约,已自动按 🪙<b>代币监控</b>添加:"
            + "、".join(auto_switched)
            + "\n该代币的<b>所有转账</b>(任何人之间)都会推送提醒。")
